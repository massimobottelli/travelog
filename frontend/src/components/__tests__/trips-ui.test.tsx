...
describe("TripsPage", () => {
  ...
  it("shows the MANUALE badge in the list and inline day editing in every trip detail", async () => {
    // Fixture: trip 1 is auto-generated (autoGenerated: true,
    // createdManually: false), trip 2 is manual (autoGenerated: false,
    // createdManually: true). The MANUALE badge is on trip 2 only; the
    // inline day editing (day/locality trash, "Aggiungi località") is
    // available in every active trip's detail (user request: on
    // auto-generated trips deletions become day exclusions).
    mockApi();
    render(<TripsPage />);
    ...
    // Open the detail of the manual trip: without entering the edit mode
    ...
    expect(
      screen.queryByRole("button", { name: "Aggiungi località al giorno 05/09/2025" }),
    ).toBeNull();

    // Enter the edit mode ("Modifica" in the header): the inline edit
    // affordances appear. With a single day its trash is disabled (the
    // backend requires at least one day).
    fireEvent.click(screen.getByRole("button", { name: "Modifica" }));
    const dayTrash = screen.getByRole("button", { name: "Elimina il giorno 05/09/2025" });
    expect(dayTrash).not.toBeNull();
    expect((dayTrash as HTMLButtonElement).disabled).toBe(true);
    expect(
      screen.getByRole("button", {
        name: "Elimina la località Roma del giorno 05/09/2025",
      }),
    ).not.toBeNull();
    expect(
      screen.getByRole("button", { name: "Aggiungi località al giorno 05/09/2025" }),
    ).not.toBeNull();

    // Close it and open the auto-generated trip: the editing is available
    // there too (deletions are persisted as day exclusions).
