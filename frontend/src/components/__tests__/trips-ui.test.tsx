...
/** Trip 2 detail: a manual trip with one manual day (Weekend a Roma). */
const TRIP_DETAIL_2 = {
  ...TRIPS.items[1],
  days: [
    {
...
/** Map payload with no GPS markers: the map panel shows its empty state. */
const EMPTY_MAP = {
  id: 1,
  name: "Vacanza in Toscana",
  startDate: "2025-08-10",
...
describe("TripsPage", () => {
  ...
  it("shows the MANUALE badge and enables the inline editing from the expanded card (§51)", async () => {
    // Fixture: trip 1 is auto-generated (autoGenerated: true,
    // createdManually: false), trip 2 is manual (autoGenerated: false,
    // createdManually: true) → the provenance badge marks trip 2 only.
    mockApi();
    render(<TripsPage />);
    ...
    expect(screen.getByText("MANUALE")).not.toBeNull();
    // The old gear button in the table column is gone.
    expect(screen.queryByTitle("Modifica giorni")).toBeNull();

    // Expanding the card reveals the day/locality timeline (new UI §2.2).
  ...
  it("persists an inline day edit of the expanded card through PUT /api/trips/:id/days (§51)", async () => {
    ...
    fetchMock.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (/^\/api\/trips\/2\/days$/.test(url) && init?.method === "PUT") {
    ...
    expect(JSON.parse(putCalls[0])).toEqual({ days: [{ date: "2025-09-05" }] });
  });

  it("offers the explicit recalculation command in the global action menu (§1.1)", async () => {
    fetchMock.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
  ...
  it("collapses the expanded card when the card header is clicked again", async () => {
    mockApi();
    ...
    const toggle = () => screen.getByRole("button", { name: /^Vacanza in Toscana/ });
    ...
    // The timeline is removed and the trip list is visible again
    await waitFor(() => {
      expect(screen.queryByText("Nessuna foto")).toBeNull();
    });
    expect(screen.getByText("Vacanza in Toscana")).not.toBeNull();
    // aria-expanded on the card header toggles back to false
  ...
  it("delete flow: confirms then issues DELETE /api/trips/:id", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Azioni per Vacanza in Toscana" }));
  ...
  it("re-queries the list server-side while typing in the search field (§1.1)", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Cerca viaggi"), { target: { value: "Roma" } });

    await waitFor(() => {
      const searches = fetchMock.mock.calls
        .map(([url]) => String(url))
        .filter((url) => url.includes("search="));
      expect(searches.some((url) => url.includes("search=Roma"))).toBe(true);
    });
  });

  it("lays out the trip list and the full-height map panel side by side (§3)", async () => {
    mockApi();
    ...
    expect(container.querySelector(".trips-dashboard")).not.toBeNull();
    expect(container.querySelector(".trips-sidebar")).not.toBeNull();
    expect(container.querySelector(".trips-map-panel")).not.toBeNull();
  });

  it("opens the shareable detail page from the card external-link button", async () => {
    mockApi();
    const openSpy = vi.fn();
    ...
    expect(openSpy).toHaveBeenCalledWith("/trips/1", "_blank");
  });
});
