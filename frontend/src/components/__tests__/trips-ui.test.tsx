...
function mockApi(): void {
  fetchMock.mockImplementation(async (input: RequestInfo | URL) => {
    ...
    if (/^\/api\/trips\/2(\?|$)/.test(url)) {
      return jsonResponse({ ...TRIPS.items[1], days: [] });
    }
...
describe("TripsPage", () => {
  ...
  it("shows the MANUALE badge in the list and 'Modifica' in the detail only for manual trips", async () => {
    ...
    // "Modifica" button lives top-right inside the detail panel.
    mockApi();
    render(<TripsPage />);
    ...
    // Open the detail of the manual trip: "Modifica" appears top-right.
    fireEvent.click(screen.getByRole("button", { name: "Weekend a Roma" }));
    await waitFor(() => {
      expect(
        screen.getByRole("button", {
          name: "Modifica i giorni del viaggio Weekend a Roma",
        }),
      ).not.toBeNull();
    });

    // Close it and open the auto-generated trip: no "Modifica" button.
    ...
    // Wait for the detail to be loaded before asserting the absence.
    await waitFor(() => {
      expect(screen.getByText(/Dettagli Viaggio: Vacanza in Toscana/)).not.toBeNull();
    });
    expect(
      screen.queryByRole("button", {
        name: "Modifica i giorni del viaggio Vacanza in Toscana",
      }),
    ).toBeNull();
  });
