...
export const trips = pgTable(
  "trips",
  {
    ...
    /** True only for trips created by the user via manual creation (POST /trips). */
    createdManually: boolean("created_manually").notNull().default(false),
...
// ── 7. Manual Trip Days ────────────────────────────────────────────
// Manual trip creation with days added one at a time (each day may
// temporarily have zero localities). Dedicated tables: presences is
// derived data and is rebuilt by recalculation. Manual days ignore
// exclusion zones.

export const manualTripDays = pgTable(
  "manual_trip_days",
  {
    id: serial("id").primaryKey(),
    ...
    dayDate: date("day_date", { mode: "date" }).notNull(),
    createdAt: timestamp("created_at", {
      mode: "date",
      withTimezone: false,
    })
  ...
  (t) => ({
    uniqueManualDay: unique("unique_manual_trip_day").on(t.tripId, t.dayDate),
    idxManualTripDaysTripId: index("idx_manual_trip_days_trip_id").on(t.tripId),
    idxManualTripDaysDayDate: index("idx_manual_trip_days_day_date").on(t.dayDate),
  }),
);

export const manualTripDayLocalities = pgTable(
  "manual_trip_day_localities",
  {
    dayId: integer("day_id")
      .notNull()
      .references(() => manualTripDays.id, { onDelete: "cascade" }),
