...
export const photos = pgTable(
  ...
  (t) => ({
    ...
    idxPhotosDateTime: index('idx_photos_date_time_original').on(
      t.dateTimeOriginal
    ),
  })
);
...
export const administrativeAreas = pgTable(
  ...
  (t) => ({
    idxAdminAreaCountryLevel: index(
      'idx_administrative_areas_country_level'
    ).on(t.countryCode, t.adminLevel),
...
export const geocodingCache = pgTable(
  'geocoding_cache',
  {
    ...
    adminAreaId: integer('admin_area_id').references(
...
export const settings = pgTable('settings', {
  id: serial('id').primaryKey(),
  minPhotoCountPerVisit: integer('min_photo_count_per_visit')
    .notNull()
    .default(1),
  daysWithoutPhotosThreshold: integer('days_without_photos_threshold')
    .notNull()
    .default(3),
  updatedAt: timestamp('updated_at', {
    mode: 'date',
